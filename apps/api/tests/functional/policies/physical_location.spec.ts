...
const actions = ['list', 'view', 'create', 'rename', 'reparent', 'archive', 'restore'] as const
...
function policyFor(effectiveAccountId: string | null) {
  ...
  return new PhysicalLocationPolicy(rootAuthority)
}

test.group('Physical location policy', () => {
  ...
  test('does not grant actions through account status or naming shortcuts', async ({ assert }) => {
    const account = {
