...
export default class extends BaseSchema {
  protected tableName = 'catalogue_item_attribute_values'
  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table.unique(['catalogue_item_id', 'category_attribute_id'], {
        indexName: 'catalogue_item_attribute_values_item_attribute_unique',
      })
      table
        .foreign(
          ['choice_id', 'category_attribute_id'],
          'catalogue_item_attribute_values_choice_attribute_fk'
        )
    ...
    this.defer(async (db) => {
      await db.rawQuery(`
        ALTER TABLE catalogue_item_attribute_values
          ADD CONSTRAINT catalogue_item_attribute_values_type_valid
            CHECK (data_type IN ('TEXT', 'NUMBER', 'DATE', 'YES_NO', 'PREDEFINED_CHOICE')),
          ADD CONSTRAINT catalogue_item_attribute_values_text_present
            CHECK (text_value IS NULL OR btrim(text_value) <> ''),
          ADD CONSTRAINT catalogue_item_attribute_values_number_finite
            CHECK (number_value IS NULL OR number_value::text NOT IN ('NaN', 'Infinity', '-Infinity')),
          ADD CONSTRAINT catalogue_item_attribute_values_typed_value_valid
            CHECK (
